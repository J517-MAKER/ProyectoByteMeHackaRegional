...
def simulate_voice_event(text='ayuda, me están siguiendo', camera_id='CAM-008'):
    ...
    # 1. Crear evento en el store local
    event = VoiceEvent(
        f'VOZ-{len(store.voice_events)+1:03d}',
        store.now(),
        camera_id,
        text,
    ...
    # 2. Guardar automáticamente la captura con vector en PostgreSQL (Docker)
    try:
        # Vector simulado de 512 dimensiones para el rostro capturado en la escena
        vector_simulado = np.random.rand(512).astype(np.float32).tolist()
        
        captura_id = guardar_captura_rostro(
            codigo_camara=camera_id,
            embedding=vector_simulado,
            ruta_foto=f'assets/capturas/alerta_{event.id}.jpg',
            tipo_evento=event.subtype,
            metadata={'frase_detectada': text, 'evento_id': event.id}
        )
        print(f"🚨 Evento de voz {event.id} guardado en PostgreSQL con ID de captura: {captura_id}")
    except Exception as e:
        print(f"⚠️ Evento registrado en interfaz, pero falló el guardado en PostgreSQL: {e}")
...
def process_voice_command(text):
    ...
    actions = {
        'iniciar busqueda': 'INICIAR_BUSQUEDA',
        'detener busqueda': 'DETENER_BUSQUEDA',
        'mostrar ultima deteccion': 'MOSTRAR_ULTIMA_DETECCION',
        'mostrar coincidencias': 'MOSTRAR_COINCIDENCIAS',
        'mostrar camaras cercanas': 'MOSTRAR_CAMARAS_CERCANAS',
        'continuar seguimiento': 'CONTINUAR_SEGUIMIENTO',
