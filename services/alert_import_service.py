...
proposals for human review, and the facial comparison itself belongs to the facial
module: this service only prepares the reference and the candidate set.
"""
import re
import unicodedata
...
def save_upload(content, content_type='', filename=''):
...
def prepare_face_reference(photo_path):
    """Normalises the cropped photo for the facial module. It does not compare faces."""
    ...
    return {'status': 'REFERENCIA_PREPARADA', 'reference': project_path(path), 'mock': True,
...
def match_face_reference(face_reference):
    """Integration point: the facial module will assign the comparison levels."""
    if not face_reference or face_reference.get('status') != 'REFERENCIA_PREPARADA':
