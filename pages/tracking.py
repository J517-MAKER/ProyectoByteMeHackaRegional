...
def tracking_page(case_id:str='BUS-2026-0184',alert_id:str='',camera_id:str=''):
    with PageLayout('/tracking','Mapa y seguimiento','Puntos de detección y relaciones cronológicas. Las conexiones no representan una trayectoria física.'):
        ...
        if not alert:
            options={c.id:f'{c.id} · {c.person.name}' for c in get_cases()}
        ...
        case=get_case(case_id) if not alert else None
        history=get_tracking_history(case.id) if case and not camera_id else []
        selected_camera=get_camera(event.camera_id if event else camera_id) if event or camera_id else None
            ...
            with Panel('Información del seguimiento'):
                with ui.column().classes('panel-body gap-1'):
                    if selected_camera:
                        ...
                        for label,value in [('Inicio del seguimiento',valid[0].detection.timestamp if valid else 'Sin detecciones'),
                                             ('Última detección',valid[-1].detection.timestamp if valid else 'Sin detecciones'),
                                             ('Tiempo entre detecciones',str(datetime.fromisoformat(valid[-1].detection.timestamp)-datetime.fromisoformat(valid[0].detection.timestamp)) if valid else '—'),
                                             ('Cámaras con detecciones',len({t.detection.camera_id for t in history})),
