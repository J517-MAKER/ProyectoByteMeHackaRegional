...
def cameras_page(camera_id:str=''):
    with PageLayout('/cameras','Red de cámaras','Vistas de prueba de la red distribuida. Selecciona una cámara para consultar su información.'):
        ...
        def open_camera(cid):
            ...
            with detail:
                ...
                with ui.column().classes('p-5 w-full gap-2'):
                    ...
                    for label,value in [('Nombre',camera.name),('Ubicación',camera.location),('Última comunicación',camera.last_seen),('Audio','Disponible · simulación' if camera.audio else 'No disponible')]:
        ...
        def grid():
            ...
            ui.label(f'{min(start+1,len(cameras))}–{min(start+count*count,len(cameras))} de {len(cameras)} cámaras · Capturas sintéticas estáticas').classes('text-xs muted mt-3')
